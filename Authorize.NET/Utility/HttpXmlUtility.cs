...
namespace AuthorizeNet
{
    ...
    public class HttpXmlUtility
    {
        ...
        void CheckForErrors(ANetApiResponse response)
        {
            ...
            {
                //there's a directResponse we need to find....-by?  <- so you can uselessly overwrite it, wtf? commented out-RC
                //var thingy = (createCustomerProfileTransactionResponse)response;
                //thingy.directResponse = _xmlDoc.ChildNodes[1].ChildNodes[0].InnerText;
                //response = thingy;
            }
            else
            {
