...
function FTB_InsertImage(galleryID, rootfolder, selectedFolder) {
    var src = document.getElementById('img_preview').src.split('?')[0];
    src=src.split('/')[src.split('/').length-1];
    if( src=='WebResource.axd' ) return;
   src=document.getElementById('img_preview').src.split('/')[document.getElementById('img_preview').src.split('/').length-1];
   targetitemed=self.name;
   src=myRoot + "/" + currentImage;
   if(targetitemed)
        window.opener.document.getElementById(targetitemed).value= src;
