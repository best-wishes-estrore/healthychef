...
<script runat="server">
	...
	public override bool CheckAuthentication()
	{
		...
		return (null != Page.User 
            && null != Page.User.Identity
            && Page.User.Identity.IsAuthenticated
            && (Page.User.IsInRole(BayshoreSolutions.WebModules.Security.Role.Administrators)
                || Page.User.IsInRole(BayshoreSolutions.WebModules.Security.Role.Content_Authors)));
	}
	...
	public override void SetConfig()
	{
        ...
        LicenseName = "Web Modules";
        LicenseKey = "4NG1-W16E-KWTG-1DJG-3DE8-ADW6-6DDJ";

		// The base URL used to reach files in CKFinder through the browser.
        // The base URL used to reach files in CKFinder through the browser.
        string strVirtualPath = HttpRuntime.AppDomainAppVirtualPath;
        if (!strVirtualPath.EndsWith("/"))
        {
            strVirtualPath += "/";
        }
        BaseUrl = string.Format("{0}userfiles/", strVirtualPath);
		...
		type.MaxSize = 0;
		type.AllowedExtensions = new string[] { "swf", "flv", "xml", "jpg" };
		type.DeniedExtensions = new string[] { };
	}
